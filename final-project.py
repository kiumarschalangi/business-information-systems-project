...
def check_conformance_rules(df, case_col, time_col, act_col):
    ...
    for case_id, group in df.groupby(case_col):
        ...
        if "Triage in the ED" in activities:
            ...
            triage_indices = [
                i for i, x in enumerate(activities) if x == "Triage in the ED"
            ]
        ...
        if (
            "Medicine dispensations" in activities
            and "Medicine reconciliation" in activities
        ):
            # Get the timestamp of the first medication given
            first_meds_time = group[group[act_col] == "Medicine dispensations"][
                time_col
            ].min()
            # Get the timestamp of the doctor's reconciliation (order)
            first_recon_time = group[group[act_col] == "Medicine reconciliation"][
                time_col
            ].min()
...
def plot_deviations(total, skips, inserts):
    ...
    plt.title(
        "Conformance Checking: Frequency of Deviations", fontsize=14, fontweight="bold"
    )
